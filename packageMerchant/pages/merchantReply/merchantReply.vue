<template>
  <view class="merchant-reply">
    ...
    <view class="merchant-section">
      ...
      <view class="transport-info" v-if="needTransport === 1">
        <view class="section-title">托运金额</view>
        <view class="transport-fee">￥{{transportFee}}</view>
      </view>
    </view>
...
export default {
  ...
  onLoad(options) {
    if (options.orderInfo) {
      try {
        ...
        this.transportFee = orderInfo.transportFee
        this.needTransport = orderInfo.needTransport
  ...
  methods: {
    ...
    enterShop() {
      ...
      uni.navigateTo({
        url: `/packageUser/pages/shop/shop?shopId=${this.shopId}&shopName=${encodeURIComponent(this.merchantName)}&repairOrderId=${this.repairOrderId}&transportFee=${this.transportFee}&needTransport=${this.needTransport}`
      })
    },
