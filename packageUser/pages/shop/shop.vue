	...
	<view class="container">
		...
		<view class="skeleton" v-if="isLoading">
			...
			<view class="search-bar">
				<view class="search-input skeleton-bg"></view>
			</view>
			...
			<view class="brand-filter">
				<view class="brand-scroll">
					<view class="brand-list">
						<view class="brand-item skeleton-bg" v-for="i in 6" :key="i"></view>
					</view>
				</view>
			</view>
			...
			<view class="main-content">
				...
				<view class="category-list">
					<view class="category-item skeleton-bg" v-for="i in 8" :key="i"></view>
				</view>
				
				<!-- 右侧商品列表骨架 -->
				<view class="product-list">
					<view class="product-items">
						<view class="product-item" v-for="i in 8" :key="i">
							<view class="product-img skeleton-bg"></view>
							<view class="product-info">
								<view class="product-info-top">
									<view class="product-name skeleton-bg"></view>
									<view class="product-stock skeleton-bg"></view>
								</view>
								<view class="product-price-wrap">
									<view class="price skeleton-bg"></view>
								</view>
							</view>
						</view>
					</view>
...
export default {
	...
	onLoad(options) {
		if (options.shopId) {
			...
			this.transportFee = options.transportFee
			this.needTransport = options.needTransport
			// 加载该商家的商品
			this.loadProducts()
		} else {
	...
	methods: {
		...
		async goToCheckout() {
			...
			try {
				let includeTransportFee = false
				// 如果有维修订单ID，则检查是否第一次购买
				if (this.repairOrderId) {
					const res = await api.order.checkFirstPurchase(this.repairOrderId)
					if (res.code !== 200) {
						uni.showToast({
							title: res.msg || '检查购买记录失败',
							icon: 'none'
						})
						return
					}
					
					// data为0表示第一次购买，为1表示不是第一次购买
					const isFirstPurchase = res.data === 0
					if (!isFirstPurchase) {
						uni.showModal({
							title: '提示',
							content: '您已购买过维修配件，是否继续购买？',
							success: (result) => {
								if (result.confirm) {
									this.proceedToCheckout(false) // 非首次购买，不计算运费
								}
							}
						})
						return
					} else {
						includeTransportFee = true // 首次购买，需要计算运费
					}
				}
				
				this.proceedToCheckout(includeTransportFee)
			} catch (error) {
		...
		// 添加处理结算的方法
		proceedToCheckout(includeTransportFee = false) {
			const orderInfo = {
				shopId: this.shopId,
				...
				transportFee: includeTransportFee ? this.transportFee : 0,
				needTransport: includeTransportFee ? this.needTransport : false
			}
			...
			uni.navigateTo({
				url: '/packageOrder/pages/checkout/checkout'
			})
		},
		...
		loadProducts() {
			this.loading = true
			this.isLoading = true // 开始加载时显示骨架屏
			api.shop.getProducts({
				...
				this.loading = false
				this.isLoading = false // 加载完成后隐藏骨架屏
			})
		},
		// 修改数据组织方法
