	...
	methods: {
		...
		goToMerchantDetails(shop) {
			uni.getLocation({
				...
				success: (res) => {
					uni.navigateTo({
						url: `/packageMerchant/pages/merchantDetails/merchantDetails?shopId=${shop.id}&latitude=${res.latitude}&longitude=${res.longitude}`
					})
				},
				fail: () => {
					// 如果获取位置失败，使用默认值0
					uni.navigateTo({
						url: `/packageMerchant/pages/merchantDetails/merchantDetails?shopId=${shop.id}&latitude=0&longitude=0`
					})
				}
			})
		},
