          ...
          <view class="order-content">
            ...
            <view class="btn-row">
              <button 
                ...
                v-if="order.status === '0' && order.statusDetail !== '2' && order.shopId"
              >
                查看回复
              </button>
            </view>
...
export default {
  ...
  methods: {
    ...
    goToOrderDetail(orderId, type) {
      if (type === 'repair') {
        uni.navigateTo({
          url: `/packageOrder/pages/orderDetail/orderDetail?orderId=${orderId}`
        })
      } else {
        uni.navigateTo({
          url: `/packageOrder/pages/repairParts/repairParts?orderId=${orderId}&type=2`
        })
      }
    }
  }
