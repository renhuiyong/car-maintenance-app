<template>
  <view class="checkout-container">
    ...
    <view class="amount-info">
      ...
      <view class="amount-item" v-if="needTransport === 1">
        <text>托运费</text>
        <text class="red-text">¥{{transportFee}}</text>
      </view>
...
export default {
  ...
  onLoad(options) {
    ...
    if (orderInfo) {
      ...
      this.totalAmount = Math.round(info.totalAmount * 100) / 100
      this.freight = info.freight || 0
      this.lastOrderAmount = this.totalAmount
      this.repairOrderId = info.repairOrderId
      this.needTransport = parseInt(info.needTransport || 0)
      this.transportFee = parseFloat(info.transportFee || 0)
      
      // 计算初始最终金额
      let tempAmount = Math.round(this.totalAmount * 100) / 100
      if (this.needTransport === 1) {
        tempAmount = Math.round((tempAmount + Number(this.transportFee)) * 100) / 100
      }
      this.finalAmount = tempAmount
  ...
  methods: {
    ...
    applyCoupon(coupon) {
      ...
      if (this.totalAmount >= coupon.minAmount) {
        ...
        // 先计算商品金额减去优惠券金额
        let tempAmount = Math.round((this.totalAmount - this.couponAmount) * 100) / 100
        // 如果需要托运，加上托运费
        if (this.needTransport === 1) {
          tempAmount = Math.round((tempAmount + Number(this.transportFee)) * 100) / 100
        }
            ...
            success: (payRes) => {
              ...
              setTimeout(() => {
                ...
                if (this.repairOrderId) {
                  uni.redirectTo({
                    url: `/packageOrder/pages/orderDetail/orderDetail?orderId=${res.data.orderNo}`
                  })
                } else {
                  uni.redirectTo({
                    url: `/packageOrder/pages/repairParts/repairParts?type=2&orderId=${res.data.orderNo}`
                  })
                }
              }, 1500)
            },
